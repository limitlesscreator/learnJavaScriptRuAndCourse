...
// конспект чтобы не потерять
// Алгоритм - Это набор последовательных действий которые решают какую-то задачу. В принципе любой фрагмент кода можно назвать алогоритмом.
//     Сложность алгоритма или его скорость описывается анатацией О(n - кол-во операций ) большое. На самом деле при вычислении алгоритмов есть несколько градаций
//     :Тета,О-большое,о-малое,Омега-большое и Омега малое. Но как правило всех интересует О-большое потому что оно будет показывать как будет меняться производительность
// алгоритма в зависимости от роста входящих данных, если мы будем увеличивать кол-во входящих данных у нас будет расти время за которое будет отрабатывать наш алгоритм и
// второе это кол-во памяти используемым данным алгоритмом для обработки всего этого объёма данных.
//     Существуют разные сложности алгоритмов например:
//     Представим, что у нас есть функция, которая принимает аргументом массив и возвращает последний  элемент данного массива такая функция работает за константное время вне
// зависимости от кол-ва данных которые нам поступают, если будет массив из 10 элементов или же из 10000 время отработки данной функции будет одинаково, это константная сложность O(1).
//
//     Второй пример – представим функцию которая считает сумму всех элементов массива который передаётся ей на входе и возвращает сумму. В данном случае внутри нашей функции будет
// цикл который будет перебирать каждый элемент массива и суммировать их значение в переменную например sum. В таком варианте у нас получается линейная зависимость времени на обработку
// алгоритма от количества данных которые мы подадим на вход. Такая сложность называется линейная, обозначается как O(n). То если 1 элемент занимает 1мс, то 100 элементов займут 100мс.
//
//     Так же знаю есть бинарный поиск с помощью данного алгоритма мы быстро и эффективно можем находить необходимою нам информацию в больших объёмах отсортированных данных. То есть важно
// знать, что данный алгоритм нужно применять к отсортированным данным. Можем представить телефонную книгу и допустим нам надо найти определённого человека. Вероятно самое первое что придёт на
// ум это просто перебрать всех с самого начала до самого конца и с каждым шагом просто сравнивать текущий элемент и искомый, алгоритм работающий, но медленный, если в телефоной книге будет 1 000 000 людей,
//     то это займёт 1 000 000 итераций в худшем случае. Но есть более эффективный способ
// То есть мы открываем наш справочник по центру и мы сможем сразу понять, имя которое мы ищем находится с правой стороны или с левой. Если мы понимаем что наш искомый человек находится в правой части,
//     то мы можем сразу убрать или удалить левую часть книги. То есть мы сразу убрали половину. С правой части книги мы повторяем ту же операцию, делим на два по центру и проверяем где будет наше имя,
//     слева или справа и так до тех пор пока не найдём наше имя. Сложность данного алгоритма  O(log от n). Если наша книга будет на 1 000 000 записей, то как я говорил  в худшем случае при линейном алгоритме это займёт 1 000 000
// итераций сложность O(n), а с алгоритмом бинарного поиска при том же кол-ве людей, нам понадобится всего 20 итераций.
...
// замыкание
это некоторый способ сохранить переменную в области видимости функции которая у нас возвращается и изменяет её значение. Например нам нужен счётчиков и чтобы не создавать кучу
// глобальных перемен начального состояния мы можем написать одну функцию внутри которой объявим count = 0, и затем будем возвращать функцию которая будет увеличивать count на единицу и возвращать его.
//     function Counter() {
//     let count = 0
//     return () => {
//         count++
